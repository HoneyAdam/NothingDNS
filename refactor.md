...
**Verification pass results (2026-05-29):** Of the items triaged so far — **D1 false positive**, **D2 intentional safety design**, **D3 & D4 fixed** (context timeout + single-lock read in replicateToFollowers), **D5 intentional & tested design** (kept, minor RFC-strictness caveat), **D7 fixed**, **D9 real & fixed**. **D10 mostly disproven** (caller already bounds-checks). So of the 10 hypotheses examined, only D9 was a genuine defect; the rest were either correct/intended or fixed. This hit rate is exactly why the doc tags everything as a hypothesis. **Remaining open: D6** (resolver source-port), **D8** (CSRF synchronizer token).
