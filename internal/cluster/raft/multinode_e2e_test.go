...
func TestMultiNode_ZoneChangeReplicatesToFollower(t *testing.T) {
	...
	ciA, err := NewClusterIntegration(idA, []NodeID{idB}, nil, addrA, t.TempDir(), "", "", nil, util.DefaultLogger())
	if err != nil {
		t.Fatalf("NewClusterIntegration A: %v", err)
	}
	ciB, err := NewClusterIntegration(idB, []NodeID{idA}, nil, addrB, t.TempDir(), "", "", nil, util.DefaultLogger())
	if err != nil {
